...
const Router = require('feliz-server-router');
...
module.exports = function server(info){ return this
    ...
    .map(() => {
        ...
        if (!this.conf.server) this.conf = Conf;
        else this.conf = this.util.object(Conf).merge({ server: this.conf.server });
    ...
    .do(serverInstance => this.events.emit(`${Name}:plugins~before`, serverInstance))
    // Are there plugins on conf? register them.
    .switchMap(serverInstance => rxPlugins
        .call(this, serverInstance)
        .toArray()
        .mapTo(serverInstance))
    .do(serverInstance => this.events.emit(`${Name}:plugins`, serverInstance))
    .do(serverInstance => this.events.emit(`${Name}:routes~before`, serverInstance))
    // Are there any routes? (ofcourse there are) register them.
    .switchMap(serverInstance => rxRoutes
        .call(this, serverInstance)
        .toArray()
        .mapTo(serverInstance))
    .do(serverInstance => this.events.emit(`${Name}:routes`, serverInstance))
